...
const CityList = ({ cities }) => {
  const { handleSelectCity } = useContext(WeatherContext);

  const Row = ({ index, style }) => (
    <div style={style} className="city-row" onClick={() => handleSelectCity(cities[index])}>
      {cities[index]}
    </div>
  );
...
const App = () => {
  ...
  const popularCities = [
    "Moscow", "London", "Tokyo", "New York", "Paris", "Berlin", "Istanbul",
    "Dubai", "Bangkok", "Singapore", "Seoul", "Rome", "Barcelona", "Madrid",
    "Amsterdam", "Toronto", "Chicago", "Los Angeles", "Mexico City", "Mumbai"
  ];
